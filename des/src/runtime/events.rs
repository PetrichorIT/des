...
#[cfg(feature = "async")]
use crate::processing::TokioRuntime;
...
impl NetEvents {
    /// Handles the execution of a network event.
    ///
    /// # Errors
    ///
    /// Errors that may occur during the handling of the network event.
    pub fn handle<A: SimLifecycle>(self, rt: &mut Sim<A>) -> Result<(), Failure> {
        match self {
...
impl MessageExitingConnection {
    ...
    fn handle<A: SimLifecycle>(self, rt: &mut Sim<A>) -> Result<(), Failure> {
...
impl HandleMessageEvent {
    fn handle<A: SimLifecycle>(self, rt: &mut Sim<A>) -> Result<(), Failure> {
        ...
        rt.error.extend(module.handle_message(message).err());
        module.deactivate();
        ...
        for stage in 0..max_stage {
            ...
            for module in &self.modules {
                ...
                if stage < module.num_sim_start_stages() {
                    ...
                    rt.error.extend(module.at_sim_start(stage).err());
                    module.deactivate();
...
impl ModuleRestartEvent {
    fn handle<A: SimLifecycle>(self, rt: &mut Sim<A>) -> Result<(), Failure> {
        ...
        module.activate_with(Some(ctx.clone()));
        rt.error.extend(module.module_restart().err());
        module.deactivate();
...
impl AsyncWakeupEvent {
    fn handle<A: SimLifecycle>(self, rt: &mut Sim<A>) -> Result<(), Failure> {
        ...
        module.activate_with(Some(ctx.clone()));
        rt.error.extend(module.async_wakeup().err());
        module.deactivate();
