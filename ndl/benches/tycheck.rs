...
fn tycheck_bench(c: &mut Criterion) {
    const KB: usize = 1024;

    let mut group = c.benchmark_group("ndl::tycheck::by_input_size");
    for size in [1, 2, 4, 8] {
        ...
        group.bench_with_input(
            ...
            |b, _size| {
                b.iter(|| {
                    tychk(&mut resolver);
                    resolver.ectx.tychecking_errors.clear();
                })
            },
        );
    }
...
criterion_group!(benches, tycheck_bench);
